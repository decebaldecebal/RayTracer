...
/*   Updated: 2015/03/11 18:18:06 by lmuresan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		get_sx_sy_aliasing2(float *sx, float *sy, int xy[2], int axy[2])
{
...
void		get_sx_sy_aliasing(float *sx, float *sy, int xy[2], int axy[2])
{
	...
	{
		*sx = ((xy[0] + (float)axy[0] / ((float)ANTIALIASING - 1)) / WIN_WIDTH)
			* ASPECT - (((WIN_WIDTH - WIN_HEIGHT) / (float)WIN_HEIGHT) / 2);
		*sy = ((WIN_HEIGHT - xy[1]) + (float)axy[0] / ((float)ANTIALIASING - 1))
			/ WIN_HEIGHT;
	}
	else if (WIN_HEIGHT > WIN_WIDTH)
	{
		*sx = ((xy[0] + (float)axy[0] / ((float)ANTIALIASING - 1)) /
			WIN_WIDTH);
		*sy = (((WIN_HEIGHT - xy[1]) + (float)axy[0] /
			((float)ANTIALIASING - 1)) / WIN_HEIGHT) / ASPECT -
			(((WIN_HEIGHT - WIN_WIDTH) / (float)WIN_WIDTH) / 2);
	}
	else
		get_sx_sy_aliasing2(sx, sy, xy, axy);
}
